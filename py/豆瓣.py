...
class Spider(Spider):
    ...
    def buildVod(self, item):
        ...
        return {
            "vod_id": "msearch:" + str(item.get("id", "")),
            "vod_name": item.get("title", ""),
            "vod_pic": pic_url,
            "vod_remarks": f"评分：{rating_val}",
        }
    ...
    def homeVideoContent(self):
        try:
            ...
            year = calendar.year
            url = f"https://movie.douban.com/j/new_search_subjects?sort=U&range=0,10&tags=&playable=1&start=0&year_range={year},{year}"
            USER_AGENTS = [
            ...
            aList = jo.get("data", [])
            for item in aList:
                rating = item.get("rate", "") if item.get("rate", "") else ""
                if rating:
                    rating = "评分：" + str(rating)
                remark = str(rating if rating else "评分：暂无")
                title = item.get("title", "")
                pic = item.get("cover", "")
                if pic:
                    pic = pic + "@Referer=https://api.douban.com/@User-Agent=" + headers.get('User-Agent', '')
                videos.append({
