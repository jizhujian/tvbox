...
class Spider(Spider):
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    if key not in name:
                        continue
                    mark_tag = item.select_one("button.hdtag")
                    mark_text = self.tag_name + " " + (mark_tag.get_text() if mark_tag else "")
                    pic = img.get("src", "")
                    a_tag = item.select_one("a")
                    sid = a_tag.get("href", "")
    ...
    def detailContent(self, ids):
        try:
            ...
            rsp.encoding = 'utf-8'
            root = BeautifulSoup(rsp.text, "html.parser")
            node = root.select("div.message.break-all > div")
            title_tag = node[0].select_one("a")
            img_tag = node[0].select_one("img")
            vod = {
                "vod_id": self.tag + "$" + vid,
            ...
            for p in info_paragraphs:
                ...
                if "分类" in content:
                    vod['type_name'] = content.replace("◎影片分类:", "")
                elif "备注" in content:
                    vod['vod_remarks'] = content.replace("◎影片备注:", "")
                elif "地区" in content:
                    vod['vod_area'] = content.replace("◎影片地区:", "")
                elif "主演" in content:
                    vod['vod_actor'] = content.replace("◎影片主演:", "")
                elif "导演" in content:
                    vod['vod_director'] = content.replace("◎影片导演:", "")
            play_from_list = []
            play_headers = root.select("div.clearfix.mt-3 > h2")
            for h2 in play_headers:
                text = h2.get_text()
                p = re.split('（', text)[0]
                play_from_list.append(p)
            vod['vod_play_from'] = "$$$".join(play_from_list) if play_from_list else ""
            play_url_list = []
            ul_list = root.select("div.clearfix.mt-3 > ul")
            for ul in ul_list:
                play_items = []
                a_list = ul.select("a")
                for a_tag in a_list:
